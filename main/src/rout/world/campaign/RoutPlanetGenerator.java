...
import static mindustry.Vars .*;
    ...
    public class RoutPlanetGenerator extends PlanetGenerator {
        ...
        Block getBlock(Vec3 position, boolean visualOnly) {
            ...
            float tdetail = Simplex.noise3d(seed, 3, 0.5, 1f / 3f, px, py + 500f, pz);
            temp = Mathf.clamp(Mathf.lerp(temp, tnoise, 0.5f) + tdetail * 0.3f);
            //cliff noise biases height but rawHeight spreads across the palette
            float cliffH = Simplex.noise3d(seed, 5, 0.6, 1f / 300f, px, py, pz) * 0.5f + 0.5f;
            float height = Mathf.lerp(rawHeight(position), cliffH, 0.25f);
        ...
        protected void generate() {
            ...
            //check positions on the map to place the player spawn at the center
            Room spawn = null;
            Seq<Room> enemies = new Seq<>();
            int enemySpawns = rand.random(1, Math.max((int) (sector.threat * 4), 1));

            roomseq.add(spawn = new Room(width / 2, height / 2, rand.random(8, 15)));

            for (int j = 0; j < enemySpawns; j++) {
                float enemyOffset = rand.range(360f);
                Tmp.v1.trns(enemyOffset, width / 3f).add(width / 2, height / 2);
            ...
            //v63-style cliffgen: elevation levels, cliff faces only at boundaries
            int[][] elev = new int[width][height];
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    Tile t = tiles.getn(x, y);
                    if (t.floor().isLiquid) continue;
                    float h = noise(x, y, 5, 0.6, 300, 1);
                    elev[x][y] = (int) ((h + 1f) * 1.5f);
                    float hole = noise(x + 700, y + 700, 3, 0.5, 80, 1);
                    if (hole < -0.3f) elev[x][y] = 0;
                }
            }

            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    if (elev[x][y] == 0) continue;
                    Tile t = tiles.getn(x, y);
                    if (t.block() != Blocks.air) continue;
                    boolean boundary = false;
                    for (int i = 0; i < 8; i++) {
                        int nx = x + Geometry.d8[i].x, ny = y + Geometry.d8[i].y;
                        if (nx < 0 || ny < 0 || nx >= width || ny >= height || elev[nx][ny] < elev[x][y]) {
                            boundary = true;
                            break;
                        }
                    }
                    float hole = noise(x + 700, y + 700, 3, 0.5, 80, 1);
                    if (boundary && hole > -0.3f) {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    Tile t = tiles.getn(x, y);
                    if (t.block() != Blocks.cliff) continue;
                    int bits = 0;
                    for (int i = 0; i < 8; i++) {
                        int nx = x + Geometry.d8[i].x, ny = y + Geometry.d8[i].y;
                        if (nx < 0 || ny < 0 || nx >= width || ny >= height || elev[nx][ny] < elev[x][y]) {
            ...
            //clear cliffs around spawn so the core is never in a pit
            Geometry.circle(spawn.x, spawn.y, tiles.width, tiles.height, 14, (x, y) -> {
                Tile t = tiles.getn(x, y);
                if (t.block() == Blocks.cliff) t.setBlock(Blocks.air);
            });
            ...
            Schematics.placeLoadout(RoutLoadouts.routerCore, spawn.x, spawn.y);

            Geometry.circle(spawn.x, spawn.y, tiles.width, tiles.height, 10, (x, y) -> {
                Tile t = tiles.getn(x, y);
                if (!t.floor().isLiquid) {
